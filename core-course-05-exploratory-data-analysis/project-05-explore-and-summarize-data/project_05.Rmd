...
```{r "Cumulated Number of Issues Created"}
...
The behavior of the project in relation to the beginning of a task activity is 
similar to the behavior of its creation. The plots are very similar. See below
the comparison of their plots.
  ...
  plot_cum_score_issue_start_date_month <-
    df_cum_score_issue_start_date_month %>% 
      ggplot(aes(x = issue_start_date, y = cumscore)) +
        ...
        coord_cartesian(ylim = c(0, 4600)) +
        labs(title = "Cumulated Number of Issues Started",
             subtitle = subtitle(nrow(df_issue_start_date_month)),
             x = "Issue start date",
             y = "Number of issues") +
        theme(axis.text.x = element_text(angle = 40, hjust = 1)) 
...
Comparison between cumulated number of issues created and started.

```{r fig.height=5.5, "Comparison between Cumulated Number of Issues Created and Started"} 
...
that there was no delay between the creation and the starting of a issue. 
