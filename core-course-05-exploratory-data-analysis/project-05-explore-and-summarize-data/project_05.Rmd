...
From here, I analyse issue logs. I start by log build info which is an attribute
of a log and represents the system's version of a future build. A log with this 
attribute indicates that there is a solution code for the issue and this code 
will be deployed on the version X of the system.
...
bizarre situation in this context was to have a version with more than 116 
issues associated.

In the plot below we can see better how is the distribution between 1 and 5 
issues.

```{r "Distribution of the Number of the Issues per System's Version (Less Than 6 Issues)"}
df_log_build_info_lt_6_issues <- 
  df_log_build_info %>% 
    filter(score < 6)
