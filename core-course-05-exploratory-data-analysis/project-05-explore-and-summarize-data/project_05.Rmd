...
```{r echo=FALSE, fig.retina=3, "Distribution of the Number of Logs per Issue"}
df_number_logs_per_issue %>% 
  plot_number_logs_per_issue()
```
  ...
  plot_cum_score_issue_created_by_month <-
    df_cum_score_issue_created_by_month %>% 
      ggplot(aes(x = issue_creation_date, y = cumscore)) +
        ...
        scale_x_date(date_breaks = "3 month", date_labels = "%b %Y",
                     expand = c(0.02, 0)) +
        scale_y_continuous(breaks = seq(0, 4600, 400),
                           sec.axis = sec_axis(
        ...
        labs(title = "Cumulated Number of Issues Created",
             subtitle = subtitle(nrow(df_issue_created_by_month)),
...
below that shows the cumulated number of issues created along the project and 
the trend line (in blue).

```{r echo=FALSE, fig.retina=3, warning=FALSE, "Cumulated Number of Issues Created"}
df_issue_created_by_month %>% 
  plot_cum_score_issue_created_by_month()
```
...
their cumulated summaries.

```{r echo=FALSE, fig.retina=3, warning=FALSE, "Distribution of the Number of Issues Created per Month"}
df_issue_created_by_month %>% 
  plot_score_issue_created_by_month()
```
...
accumulated mean variates between 10 and 20 issues per month. At the end of 2013 
...
quartile and accumulated median. This means that these months had a high number 
...
During the week, the activities were concentrated on business days (from Monday to
Friday) as can be noted in the barplot below.

```{r echo=FALSE, fig.retina=3, "Frequency of Issues Created per Weekday"}
plot_score_issue_created_by_wday(df_issue_created_by_wday)
```

...

```{r echo=FALSE, fig.retina=3, "Distribution of the Number of Issues Created per Weekday (No Weekend)"}
df_issue_created_by_wday %>% 
  filter(issue_creation_wday != "Sat" & issue_creation_wday != "Sun") %>% 
  plot_score_issue_created_by_wday("without the weekend")
```

```{r echo=FALSE, fig.retina=3, "Distribution of the Number of Issues Created per Hour of the Day"}
...
print(rbind(x = summ_number_issue_created_by_hour))
print(plot_issue_created_by_hour)
```

### Issue Start Date

```{r echo=FALSE, fig.retina=3, "Distribution of the Number of Issues Started per Month"}
...
plot_cum_score_issue_start_date_month <-
  df_cum_score_issue_start_date_month %>% 
    ggplot(aes(x = issue_start_date, y = cumscore)) +
      ...
      labs(title = "Cumulated Number of Issues Started per Month",
           subtitle = subtitle(nrow(df_issue_start_date_month)),
           x = "Issue start date (per month)",
           y = "Number of issues") +
      theme(axis.text.x = element_text(angle = 40, hjust = 1)) 

print(plot_cum_score_issue_start_date_month)
print(plot_score_issue_start_date_month)
```

```{r echo=FALSE, fig.retina=3, "Distribution of the Number of Issues Started per Weekday"}
df_issue_start_wday <- 
  issues %>% 
    filter(!is.na(issue_start_date)) %>% 
    distinct(issue_id, 
...
print(rbind(y = summ_issue_start_wday))
print(plot_score_issue_start_wday)
```

### Issue Deadline Date

```{r echo=FALSE, fig.retina=3, "Distribution of the Number of Issues Deadline per Month"}
...
plot_cum_score_issue_deadline_date_month <-
  df_cum_score_issue_deadline_date_month %>% 
    ggplot(aes(x = issue_deadline_date, y = cumscore)) +
      ...
      labs(title = "Cumulated Number of Issues Deadline per Month",
           subtitle = subtitle(nrow(df_issue_deadline_date_month)),
           x = "Issue deadline date (per month)",
           y = "Number of issues") +
      theme(axis.text.x = element_text(angle = 40, hjust = 1)) 

print(plot_cum_score_issue_deadline_date_month)
print(plot_score_issue_deadline_date_month)
```

```{r echo=FALSE, fig.retina=3, "Distribution of the Number of Issues Deadline per Weekday"}
...
```{r echo=FALSE, fig.retina=3, "Distribution of the Number of Time Spent in a Issue per Hour"}
...
```{r echo=FALSE, fig.retina=3, message=FALSE, "Distribution of the Number of Time Spent in a Issue per Hour (Zoomed In)"}
plot_issue_time_spent_hour + 
...
```{r echo=FALSE, fig.retina=3, "Distribution of the Number of Time Spent in a Issue (Less Than 1 Hour)"}
...
```{r echo=FALSE, fig.retina=3, "Distribution of the Number of Time Spent in a Issue (Less Than 1 Hour and Non  0)"}
df_issue_time_spent_min <- 
  issues %>% 
    filter(!is.na(issue_time_spent)) %>%
    distinct(issue_id, issue_time_spent = (issue_time_spent / 60)) %>% 
...
```{r echo=FALSE, fig.retina=3, "Distribution of the Number of Time Spent in a Issue (Non 0)"}
df_issue_time_spent_hour <- 
  issues %>% 
    filter(!is.na(issue_time_spent)) %>%
    distinct(issue_id, 
...
```{r echo=FALSE, fig.retina=3, "Distribution of the Priority Number per Issue"}
...
```{r echo=FALSE, fig.retina=3, "Distribution of the Priority Number per Issue (Dropped 999 and Zoomed In to 100)"}
df_issue_priority_number <- 
  issues %>% 
    filter(!is.na(issue_priority_number)) %>%
    distinct(issue_id, issue_priority_number) %>% 
...
```{r echo=FALSE, fig.retina=3, "Distribution of the Priority Number per Issue (Less than or Equal to 20)"}
df_issue_priority_number <- 
  issues %>% 
    filter(!is.na(issue_priority_number)) %>%
    distinct(issue_id, issue_priority_number) %>% 
...
```{r echo=FALSE, fig.retina=3, "Distribution of the Progress per Issue"}
df_issue_progress <- 
  issues %>% 
...
```{r echo=FALSE, fig.retina=3, "Distribution of the Number of the Issues per System's Version"}
df_log_build_info <-
  logs %>% 
...
```{r echo=FALSE, fig.retina=3, "Distribution of the Number of the Issues per System's Version (Less Than 6 Issues)"}
df_log_build_info <- 
  logs %>% 
    filter(!is.na(log_build_info)) %>% 
    distinct(issue_id, log_build_info) %>% 
