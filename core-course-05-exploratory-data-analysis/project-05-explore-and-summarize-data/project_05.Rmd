...
summ.number_logs_per_issue <- df_summary(df.number_logs_per_issue, score)
# Count number of issues for each number of logs per issue
df.number_logs_per_issue %>% 
  ggplot(aes(x = score)) +
    geom_histogram(binwidth = 1, color = "gray", alpha = .3, 
                   boundary = summ.number_logs_per_issue$min, 
                   closed = "left") +
    plot_summary(df.number_logs_per_issue, score) +
    scale_x_continuous(breaks = seq(0, 43, 2)) +
    scale_y_continuous(breaks = seq(0, 800, 50)) +
    labs(title = "Distribution of Number of Logs per Issue",
         subtitle = text_df_summary(summ.number_logs_per_issue),
         x = "Number of logs per issue",
         y = "Frequency", 
         caption = paste0("\nNumber of observations: ",
                          nrow(df.number_logs_per_issue))) +
...
summ.issue_created_by_month <- df_summary(df_score_issue_created_by_month, score)
...
df_score_issue_created_by_month %>% 
  ggplot(aes(x = issue_creation_date, y = score)) +
    ...
    labs(title = "Distribution of Number of Issues Created per Month",
         subtitle = paste("", text_df_summary(summ.issue_created_by_month)),
...
```{r echo=FALSE, fig.retina=2, "Distribution of Number of Issues Created per Weekday"}
...
```{r echo=FALSE, fig.retina=2, "Distribution of Number of Issues Created per Hour of the Day"}
df_issue_created_by_hour <- 
  issues %>% 
    distinct(issue_id, 
             issue_creation_hour = hour(issue_creation_date))

summ.number_issue_created_by_hour <- df_summary(df_issue_created_by_hour, 
                                                issue_creation_hour)

df_issue_created_by_hour %>% 
  ggplot(aes(x = issue_creation_hour)) +
    geom_histogram(binwidth = 1, color = "gray", alpha = .3, 
                   boundary = summ.number_issue_created_by_hour$min, 
                   closed = "left") +
    plot_summary(df_issue_created_by_hour, issue_creation_hour) +
    scale_x_continuous(breaks = seq(0, 23, 1)) +
    scale_y_continuous(limits = c(0, 700), breaks = seq(0, 700, 50)) +
    labs(title = "Frequency of Issues Created per Hour of the Day",
         subtitle = text_df_summary(summ.number_issue_created_by_hour),
         x = "Hour",
         y = "Frequency", 
         caption = paste0("\nNumber of observations: ",
...
df_issue_time_spent_min %>%
  plot_histogram(x = issue_time_spent, binwidth = 10,
                 x.breaks = seq(0, 60, 10), y.breaks = seq(0, 1700, 100),
                 labs.title = "Distribution of Time Spent per Issue", 
                 labs.x = "Issue time spent (minutes)",
                 labs.caption = paste0("\nNumber of observations: ",
                                       nrow(df_issue_time_spent_min)))

```

```{r echo=FALSE, fig.retina=2, "Distribution of Time Spent per Issue (Less than 1 Hour and Dropped 0 Seconds)"}
df_issue_time_spent_min <- 
  issues %>% 
    distinct(issue_id, issue_time_spent = period_to_seconds(issue_time_spent) / 60) %>% 
...
df_issue_time_spent_min %>%
  plot_histogram(x = issue_time_spent, binwidth = 10,
                 x.breaks = seq(6, 60, 10), y.breaks = seq(0, 500, 50),
                 labs.title = "Distribution of Time Spent per Issue", 
                 labs.x = "Issue time spent (minutes)",
                 labs.caption = paste0("\nNumber of observations: ",
                                       nrow(df_issue_time_spent_min)))
...
```{r echo=FALSE, fig.retina=2, "Distribution of Time Spent per Issue (Dropped 0 Seconds)"}
df_issue_time_spent_hours <- 
  issues %>% 
    distinct(issue_id, 
...
```{r echo=FALSE, fig.retina=2, "Distribution of Priority Number per Issue"}
df_issue_priority_number <- 
  issues %>% 
    distinct(issue_id, issue_priority_number)

df_issue_priority_number %>% 
  plot_histogram(x = issue_priority_number, binwidth = 50,
                 x.breaks = seq(0, 999, 50), y.breaks = seq(0, 2700, 150),
                 labs.title = "Distribution of Priority Number per Issue", 
                 labs.x = "Issue priority number",
                 labs.caption = paste0("\nNumber of observations: ",
...
```{r echo=FALSE, fig.retina=2, "Distribution of Priority Number per Issue (Dropped 999 and Zoomed In to 100)"}
df_issue_priority_number <- 
  issues %>% 
    distinct(issue_id, issue_priority_number) %>% 
...
df_issue_priority_number %>% 
  plot_histogram(x = issue_priority_number, binwidth = 10,
                 x.breaks = seq(0, 280, 10), y.breaks = seq(0, 1600, 150),
                 xlim = c(0, 100),
                 labs.title = "Distribution of Priority Number per Issue", 
                 labs.x = "Issue priority number",
                 labs.caption = paste0("\nNumber of observations: ",
...
```{r echo=FALSE, fig.retina=2, "Distribution of Priority Number per Issue (Less than or Equal to 20)"}
df_issue_priority_number <- 
  issues %>% 
    distinct(issue_id, issue_priority_number) %>% 
...
df_issue_priority_number %>% 
  plot_histogram(x = issue_priority_number, binwidth = 1,
                 x.breaks = seq(0, 20, 1), y.breaks = seq(0, 450, 50),
                 labs.title = "Distribution of Priority Number per Issue", 
                 labs.x = "Issue priority number",
                 labs.caption = paste0("\nNumber of observations: ",
                                       nrow(df_issue_priority_number)))
...
```{r echo=FALSE, fig.retina=2, "Frequency of Issue Priority Scale"}
df_issue_priority_scale <-
  issues %>%
    distinct(issue_id, issue_priority_scale)


### Ordenar pelo fator e não pelo valor
...
df_issue_priority_scale %>% 
  ...
  summarise(score = n()) %>% 
  arrange(desc(score)) %>% 
  plot_frequency.factor(x = issue_priority_scale, y = score,
                        x.breaks = seq(0, 1, .1),
                        y.limits = c(0, .7),
                        labs.title = "Frequency of Issue Priority Scale",
                        labs.x = "Issue priority scale",
                        labs.caption = paste0("\nNumber of observations: ",
...
```{r echo=FALSE, fig.retina=2, "Distribution of Progress per Issue"}
df_issue_progress <- 
  issues %>% 
    distinct(issue_id, issue_progress)

df_issue_progress %>% 
  plot_histogram(x = issue_progress, binwidth = 10,
                 x.breaks = seq(0, 100, 10), y.breaks = seq(0, 4600, 500),
                 labs.title = "Distribution of Priority Number per Issue", 
                 labs.x = "Issue priority number",
                 labs.caption = paste0("\nNumber of observations: ",
...
```{r echo=FALSE, fig.retina=2, "Distribution of Number of the Issues per System's Version"}
df_log_build_info <- 
...
df_log_build_info %>% 
  plot_histogram(x = score, binwidth = 5,
                 x.breaks = seq(1, 125, 5), y.breaks = seq(0, 320, 20),
...
```{r echo=FALSE, fig.retina=2, "Distribution of Number of the Issues per System's Version (Less Than 6 Issues)"}
df_log_build_info <- 
  subset(issues, !is.na(log_build_info)) %>% 
    distinct(issue_id, log_build_info) %>% 
    group_by(log_build_info) %>% 
...
df_log_build_info %>% 
  plot_histogram(x = score, binwidth = 1,
                 x.breaks = seq(1, 5, 1), y.breaks = seq(0, 90, 10),
                 labs.title = "Distribution of Number of the Issues per System's Version", 
                 labs.x = "Number of issues",
                 labs.caption = paste0("\nNumber of observations: ",
                                       nrow(df_log_build_info)))
