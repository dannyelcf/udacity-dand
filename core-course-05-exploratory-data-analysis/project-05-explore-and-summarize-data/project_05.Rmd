...
resources management at Universidade Federal de Goiás ([UFG](https://www.ufg.br)).
...
issue variables and ending with issue's log variables. In each variable group 
(issue and log) I begin with exploration of temporal variables and end with the 
analysis of categorical and range variables.
...
of the sum of `log_time_spent`. In the data set, issue time spent is represented 
...
df_issue_created_by %>% 
  ...
  plot_frequency(issue_created_by, 
                 title = "Top 15 Frequency of Issue's Creator",
...
Log progress is an attribute of a log that represents the rate of progress of 
the completion of one activity. Its range value is 0% (lowest) and 100% 
(highest).

The plot below shows the distribution of the log progress by issue's log order.
...
Analysing the plot above, we may note that the most of log progress values vary 
among 0%, 90% and 100%. Most log progress variability occurs in the first three 
log. And, as the order of logs increases, the progress tends to be high (it 
tends to 100%).
...
df_log_time_spent <- 
  logs %>% 
    select(issue_id, log_time_spent) %>% 
    filter(!is.na(log_time_spent)) %>%
    mutate(log_time_spent = log_time_spent / (60 * 60)) %>% 
    arrange(issue_id, log_time_spent) %>% 
    group_by(issue_id) %>% 
    mutate(log_order = paste0(stringr::str_pad(row_number(issue_id), 
                                               2, "left", "0")))
```

Log time spent is an attribute of a log that represents the total time spent 
(in seconds) to solving one activity to the issue.

The plot below shows the distribution of the log time spent (in hours) by each 
issue's log order.

```{r "Distribution of the Log Time Spent by Issue's Log Order"}
df_log_time_spent %>% 
  plot_boxplot(log_order, log_time_spent,
               title = "Distribution of the Log Time Spent by Issue's Log Order",
               label.x = "Log order",
               label.y = "Log time spent (per hour)",
               coord.ylim = c(0, 16), 
               breaks.y = seq(0, 16, 1),
               axis.text.x = element_text(angle = 45, hjust = 1))
```

Seeing the plot above we may note that there are only first fourteen logs order 
in the x axis. This means that only the first fourteen logs have log time spent
information. Now, observing the boxplots, we note that time spent to solving one 
activity increases as the log order increases. This may means that the initial 
activities are easier than middle and final activities.
...
df_log_created_by <- 
  logs %>% 
    select(issue_id, log_created_by) %>% 
    filter(!is.na(log_created_by)) %>%
    arrange(issue_id, log_created_by) %>% 
    group_by(issue_id) %>% 
    mutate(log_order = paste0(stringr::str_pad(row_number(issue_id), 
                                               2, "left", "0")),
           log_created_by = first_last_names(log_created_by)) 

df_score_top15_log_created_by <- 
  df_log_created_by %>%
    group_by(log_created_by) %>% 
    summarise(score = n()) %>% 
    arrange(desc(score)) %>% 
    ungroup() %>% 
    filter(row_number() <= 15)
...
Log created by represents the name of person who created the log in the issue. 
The plot below shows the top 15 log creators in the project.

```{r fig.height=4.5, "Top 15 Frequency of Log's Creator"}
df_log_created_by %>% 
  filter(log_created_by %in% 
           df_score_top15_log_created_by$log_created_by) %>%
  plot_frequency(log_created_by, 
                 title = "Top 15 Frequency of Log's Creator",
                 label.x = "Log creator",
                 breaks.y = seq(0, 3000, 200),
                 breaks.2nd.y = seq(0, 1, .025),
                 axis.text.x = element_text(angle = 20, hjust = 1))
...
Analysing the plot above we note that RAMON RODRIGUES is whom registered the 
most of the logs. He is also the 1st top issue creator, as we saw in the 
[Issue Created By](#issue-created-by) section. But differently of the issue 
created by, the log created by has as 2nd, 3rd and 4th top position COMPANY 
employees. This may means that issues frequently pass by one of these employees 
before return to the CUSTOMER.
...
df_has_svn_revision <- 
  logs %>% 
    select(issue_id, log_svn_revision) %>%
    mutate(has_svn_revision = if_else(condition = is.na(log_svn_revision),
                                      true = FALSE, false = TRUE)) %>% 
    group_by(issue_id) %>% 
    summarise(has_svn_revision = max(has_svn_revision)) %>% 
    mutate(has_svn_revision = if_else(condition = (has_svn_revision == 0),
                                      true = "No", false = "Yes")) %>% 
    arrange(issue_id) 
```

Log SVN revision represents the number of SVN revision when an activity involved 
changes in the source code of the subsystem.

The plot bellow shows the frequency of issues with one or more logs whith SVN 
revision number.

```{r "Frequency of Issues with SVN Revision"}
df_has_svn_revision %>% 
  plot_frequency(has_svn_revision, 
                 title = "Frequency of Issues with SVN Revision",
                 label.x = "Has SVN Revision?",
                 breaks.y = seq(0, 4000, 300),
                 breaks.2nd.y = seq(0, 1, .1))
```
...
We note that about only 25% of issues solution involved changes in the source 
code of a subsystem.

### Reflections on Initial Descriptive Analysis

Here I finish the initial descriptive analysis. We have seen the structure of the
data set and the behavior of each variable in isolation. We observed patterns and
trends in issues creation along the project. We observed also that the most 
of issues are of the MAINTENANCE type; SIGAA system; GRADUAÇÃO subsytem; HIGH
priority scale; and spent about 1h to end. Moreover, we have seen that there is
a pattern in issues solution's flow, flow size and time lapse among issue's log.

In the next section, I have analysed the association among two or more variables
of the data set.
...
#### Log Status X Log Created By

#### Issue Creation Date X Issue Subsystem
...
#### Issue Creation Date X Issue End Date (CFD)

#### Issue Creation Date X Issue End Date (Scatter)

#### (Issue Creation Date - Issue End Date) X Issue Subsystem

#### Issue Deadline Date - Issue End Date (Chart positive/negative)