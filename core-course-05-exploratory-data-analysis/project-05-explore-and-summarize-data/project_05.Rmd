  ...
  plot_score_issue_subsystem <- 
    df_score_issue_subsystem %>% 
      ggplot(aes(x = reorder(issue_subsystem, -score), y = score)) +
        ...
        labs(title = "Top 15 Frequency of Issue Subsystem",
...
```{r}
plot_score_issue_stakeholder <- function(df_issue_stakeholder,
                                         subtitle_complement = NULL) {
  
  summ_issue_stakeholder <- 
  ...
  plot_score_issue_stakeholder <- 
    df_score_issue_stakeholder %>% 
      ggplot(aes(x = reorder(issue_stakeholder, -score), y = score)) +
        geom_col(color = DEFAULT_COLOR, alpha = DEFAULT_ALFA) +
        plot_y_summary(df_issue_stakeholder, issue_stakeholder) +
        scale_y_continuous(breaks = seq(0, 3500, 250),
                           sec.axis = sec_axis(
                             trans = ~. / nrow(df_issue_stakeholder),
                             breaks = seq(0, 1, .1),
                             labels = scales::percent
                           )) +
...
```{r}
plot_score_issue_stakeholder_creator <- function(df_issue_stakeholder_creator,
                                                 subtitle_complement = NULL) {
  ...
  df_score_issue_stakeholder_creator <- 
    df_issue_stakeholder_creator %>% 
      group_by(issue_stakeholder) %>% 
      summarise(score = n())
  
  plot_score_issue_stakeholder_creator <- 
    df_score_issue_stakeholder_creator %>% 
      ggplot(aes(x = reorder(issue_stakeholder, -score), y = score)) +
        geom_col(color = DEFAULT_COLOR, alpha = DEFAULT_ALFA) +
        plot_y_summary(df_issue_stakeholder_creator, issue_stakeholder) +
        scale_y_continuous(breaks = seq(0, 150, 10),
                           sec.axis = sec_axis(
                             trans = ~. / nrow(df_issue_stakeholder_creator),
                             breaks = seq(0, 1, .1),
                             labels = scales::percent
                           )) +
        labs(title = "Frequency of Distinct Users by Stakeholder",
             subtitle = subtitle(nrow(df_issue_stakeholder_creator),
                                 subtitle_complement,
                                 summ_issue_stakeholder),
             x = "Stakeholder",
             y = "Frequency")
...
than COMPANY, but the proportion of people in each team is inversely
proportional. The plot below shows the number of distinct users by stakeholder 
who have created at least one issue related to the project.

```{r "Frequency of Distinct Users by Stakeholder"}
plt_score_issue_stakeholder_creator <-
  df_issue_stakeholder_creator %>% 
    plot_score_issue_stakeholder_creator()
...
by user we note that the first top user is from COMPANY following by 7 users 
from CUSTOMER.

```{r}
plot_score_issue_created_by <- function(df_issue_created_by,
                                        subtitle_complement = NULL) {
  
  summ_issue_created_by <- 
    df_issue_created_by %>% 
      group_by(issue_created_by) %>% 
      summarise(frequency = n()) %>% 
      df_summary(frequency)
  
  df_score_issue_created_by <- 
    df_issue_created_by %>% 
      group_by(issue_created_by) %>% 
      summarise(score = n())
  
  plot_score_issue_created_by <- 
    df_score_issue_created_by %>% 
      ggplot(aes(x = reorder(issue_created_by, -score), y = score)) +
        geom_col(color = DEFAULT_COLOR, alpha = DEFAULT_ALFA) +
        plot_y_summary(df_issue_created_by, issue_created_by) +
        scale_y_continuous(breaks = seq(0, 1400, 100),
                           sec.axis = sec_axis(
                             trans = ~. / nrow(df_issue_created_by),
                             breaks = seq(0, 1, .025),
                             labels = scales::percent
                           )) +
        labs(title = "Top 15 Frequency of Issue by Creator",
...
The first top issue creator is the Product Owner of the COMPANY in the project 
in question. He was the main contact in the COMPANY. Many issues created by him 
were reported by CUSTOMER (through Skype) and he created many issues to 
distribute activities to the COMPANY team. These are the reasons why RAMON 
RODRIGUES is the top issue creator.
