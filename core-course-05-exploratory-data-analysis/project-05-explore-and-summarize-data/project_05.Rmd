  ...
  plot_issue_delay_start <-
    df_issue_delay_start %>% 
      ggplot(aes(x = issue_delay_start)) +
        geom_histogram(binwidth = 7, color = "gray", alpha = .3, 
        ...
        labs(title = "Distribution of the Number of Days to Start an Issue",
...
```{r "Distribution of the Number of Days to Start an Issue"}
df_issue_delay_start %>% 
  plot_issue_delay_start()
```
...
it has some peculiarities. Issue deadline date has more `NA` values than those, 
approximately 
`r round(nrow(issues %>% filter(is.na(issue_deadline_date)))*100/nrow(issues))`%
of the number of issues. Its cumulative number of issues deadline it is a bit 
different from the others cumulative plots. See the plot below.
