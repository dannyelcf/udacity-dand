...
with the analysis of categorical and range variables.
...
plot_number_logs_per_issue <- function(df_number_logs_per_issue) {
  ...
  plot_number_logs_per_issue <-
    df_number_logs_per_issue %>% 
      ggplot(aes(x = score)) +
        geom_histogram(binwidth = 1, color = "grey50", alpha = .1, 
...
the red shading represents the 95% confidence interval of mean (not visible in 
this plot because its narrow interval) and the black dotted line represent the 
...
history logs (1 standard deviation from the mean), 75% of them have up to 6 
history logs and 50% of issues have between 2 and 6 logs, a narrow range. 
Despite the skewed shape of plot, the median and mean are very close. This means 
that the amount of ouliers (after 12 logs per issue) is very low, approximately 
`r amount_upper_outiler`% of issues.
...
<!--
The next 6 sections show the analisys of the temporal issues variables: four 
...
tracking system, temporal variables is the fundamental piece for monitoring and 
  ...
  plot_cum_score_issue_created_by_month <-
    df_cum_score_issue_created_by_month %>% 
      ggplot(aes(x = issue_creation_date, y = cumscore)) +
        geom_area(color = "grey50", alpha = .1) +
...
data and solid black vertical lines represent the 1st day of year.
...
cumulated median and mean, the red shading represents the 95% confidence interval 
of cumulated mean and solid black vertical lines represent the 1st day of year.

Analysing the area plot above, we note that the year 2013 had the lowest demand. 
The cumulated mean variates around 15 issues per month. At the end of 2013 and 
...
activities. The cumulated mean increases from 15 to 50 issues per month and it 
overcame the cumulated median. The distance between cumulated 3rd quartile and 
cumulated median became greater than distance between cumulated 1st quartile and 
cumulated median. The 95% confidence interval of cumulated mean also increases 
indicating an increase in the variability in the data history. All this 
indicates that these months had a high number of issues created in relation to 
the past. 
  ...
  plot_score_issue_created_by_wday <- 
    df_score_issue_created_by_wday %>% 
      ggplot(aes(x = issue_creation_wday, y = score)) +
        geom_col(color = "grey50", alpha = .1) +
        plot_frequency_ci(df_score_issue_created_by_wday, 
                          issue_creation_wday, score) +
...
> **Note:** In the plot above, the black dashed lines represent the 1st and 3rd 
quartile, the black and red solid lines represent, respectivaly, median and mean
and the red shading represents the 95% confidence interval of mean .
...
down, increase the standard deviation and the 95% confidence interval of mean . 
Removing them, we have a new barplot. See it below.
...
distance between 1st and 3rd quartile became narrow, the standard deviation and 
the 95% confidence interval of mean  descrease. These new summaries better 
reflect the behavior of issue creation.
...
> **Note:** In the plot above, the black dashed lines represent the 1st and 3rd 
quartile, the black and red solid lines represent, respectivaly, median and mean,
the red shading represents the 95% confidence interval of mean  and the black 
dotted lines represent the lower and upper threshold to the outliers 
  ...
  plot_issue_delay_start <-
    df_issue_delay_start %>% 
      ggplot(aes(x = issue_delay_start)) +
        geom_histogram(binwidth = 5, color = "grey50", alpha = .1, 
...
quartile, the black and red solid lines represent, respectivaly, median and mean,
the red shading represents the 95% confidence interval of mean  and the black 
dotted lines represents the upper threshold to the outliers 
(3rd qu. + 1.5 IQR).
...
Note also that 95% of the mean confidence interval is very narrow even though
the standard deviation to be high and the dataset contain outliers with high 
number of calendar days. This indicates that the affirmative of the previous 
paragraph is robust.
...
does not rise again. The same occur with cumulated summaries. This phenomenon 
  ...
  plot_score_issue_deadline_wday <- 
    df_score_issue_deadline_wday %>% 
      ggplot(aes(x = issue_deadline_wday, y = score)) +
        geom_col(color = "grey50", alpha = .1) +
        plot_frequency_ci(df_score_issue_deadline_wday, 
                          issue_deadline_wday, score) +
...
The issue deadline size represents the total planned time to solve and deliver 
an issue, that is, the difference between issue start and deadline date. This 
variable is interesting because it gives us an idea of the planning of the 
activities.
  ...
  plot_issue_time_spent_hour <- 
    df_issue_time_spent_hour %>% 
      ggplot(aes(x = issue_time_spent)) +
        geom_histogram(binwidth = binwidth, color = "grey50", alpha = .1, 
...
quartile, the black and red solid lines represent, respectivaly, median and mean,
the red shading represents the 95% confidence interval of mean  and the black 
dotted line represent the upper threshold to the outliers (3rd qu. + 1.5 IQR).
...
> **Note:** In the plot above, the black dashed lines represent the 1st and 3rd 
quartile, the black and red solid lines represent, respectivaly, median and mean,
and red shading represents the 95% confidence interval of mean .
  ...
  plot_score_issue_type <- 
    df_score_issue_type %>% 
      ggplot(aes(x = reorder(issue_type, -score), y = score)) +
        geom_col(color = "grey50", alpha = .1) +
        plot_frequency_ci(df_score_issue_type, 
                          issue_type, score) +
...
issue creations timeline by system/subsystem may answer that assumption.
...
plot_score_issue_system <- 
  df_score_issue_system %>% 
    ggplot(aes(x = reorder(issue_system, -score), y = score)) +
      geom_col(color = "grey50", alpha = .1) +
      plot_frequency_ci(df_score_issue_system, 
                        issue_system, score) +
...
plot_score_top15_issue_subsystem <- 
  df_score_top15_issue_subsystem %>% 
    ggplot(aes(x = reorder(issue_subsystem, -score), y = score)) +
      geom_col(color = "grey50", alpha = .1) +
      plot_frequency_ci(df_score_top15_issue_subsystem, 
                        issue_subsystem, score) +
...
plot_score_issue_stakeholder <- 
  df_score_issue_stakeholder %>% 
    ggplot(aes(x = reorder(issue_stakeholder, -score), y = score)) +
      geom_col(color = "grey50", alpha = .1) +
      plot_frequency_ci(df_score_issue_stakeholder, 
                        issue_stakeholder, score) +
      # scale_y_continuous(limits = c(0, .85), breaks = seq(0, 1, .1), 
      #                    labels = scales::percent) + 
...
plot_score_top15_issue_created_by <- 
  df_score_top15_issue_created_by %>% 
    ggplot(aes(x = reorder(issue_created_by, -score), y = score)) +
      geom_col(color = "grey50", alpha = .1) +
      plot_frequency_ci(df_score_top15_issue_created_by, 
                        issue_created_by, score) +
...
plot_issue_priority_number <- 
  df_issue_priority_number %>% 
    ggplot(aes(x = issue_priority_number)) +
      geom_histogram(binwidth = 50, color = "grey50", alpha = .1, 
...
plot_issue_priority_number <- 
  df_issue_priority_number %>% 
    ggplot(aes(x = issue_priority_number)) +
      geom_histogram(binwidth = 10, color = "grey50", alpha = .1, 
                     boundary = summ_issue_priority_number$min, 
                     closed = "left") +
      plot_x_summary(df_issue_priority_number, issue_priority_number) +
