...
```{r fig.height=4.5, "Heatmap of the Log's Creator by Issue's Log Order"}
df_log_created_by %>% 
  inner_join(df_score_top15_log_created_by, by="log_created_by") %>%
  group_by(log_created_by, score, log_order) %>% 
  count() %>% 
  filter(log_order %in% paste0(stringr::str_pad(seq(1, 20, 1), 
                                                2, "left", "0"))) %>% 
  arrange(desc(score)) %>% 
  ggplot(aes(reorder( log_created_by, -score), log_order)) +
