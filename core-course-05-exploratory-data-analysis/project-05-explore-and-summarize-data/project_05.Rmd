...
```{r echo=FALSE, fig.retina=2, "Distribution of Log Build Info"}
df_log_build_info <- 
  subset(issues, !is.na(log_build_info)) %>% 
    distinct(issue_id, log_build_info) %>% 
    arrange(log_build_info)

df_log_build_info %>% 
  group_by(log_build_info) %>% 
  summarise(score = n()) %>% 
  arrange(desc(score)) %>%
  plot_histogram(x = score, binwidth = 10,
                 x.breaks = seq(0, 200, 10), y.breaks = seq(0, 4600, 500),
                 labs.title = "Distribution of Priority Number per Issue", 
                 labs.x = "Issue priority number",
                 labs.caption = paste0("\nNumber of observations: ",
