...
df.number_logs_per_issue %>%
  ...
  plot_frequency.numeric(x = number_logs_per_issue, y = score,
                         summary = df_summary(df.number_logs_per_issue),
...
issues %>% 
  distinct(issue_id, issue_creation_date) %>% 
...
subset(issues, !is.na(issue_start_date)) %>% 
  distinct(issue_id, issue_start_date) %>% 
  group_by(issue_start_date = as.Date(floor_date(issue_start_date, "month"))) %>% 
  summarise(score = n()) %>% 
...
subset(issues, !is.na(issue_deadline_date)) %>% 
  distinct(issue_id, issue_deadline_date) %>% 
  group_by(issue_deadline_date = as.Date(floor_date(issue_deadline_date, "month"))) %>% 
  summarise(score = n()) %>% 
...
issues %>%
  ...
  plot_frequency.factor(x = issue_subsystem, y = score,
                        x.rotate = 18, y.limits = c(0, .45),
...
issues %>%
  ...
  arrange(desc(score)) %>% 
  filter(row_number() <= 15) %>% 
  mutate(issue_created_by = name_abbreviation(issue_created_by)) %>% 
  plot_frequency.factor(x = issue_created_by, y = score,
                        x.rotate = 18, y.limits = c(0, .25),
...
issues %>%
  ...
  plot_frequency.factor(x = issue_status, y = score,
                        x.breaks = seq(0, 1, .1),
                        x.rotate = 13, y.limits = c(0, 1),
                        labs.title = "Frequency (Top 15) of Issue Status",
                        labs.x = "Issue status")
```
