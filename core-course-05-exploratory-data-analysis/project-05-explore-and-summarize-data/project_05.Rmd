...
the cumulative number of issues started along the project and its trend line 
(in blue) in the plot below.
  ...
  plot_issue_delay_start <-
    df_issue_delay_start %>% 
      ggplot(aes(x = issue_delay_start)) +
        ...
        labs(title = "Distribution of the Delay Days Between Creation and Start Date",
...
```{r "Distribution of the Delay Days Between Creation and Start Date"}
...
> **Note:** In the plot above, the black dashed lines represent the 1st and 3rd 
quartile, the black and red solid lines represent, respectivaly, median and mean,
the red shading represents the mean confidence interval (95%) and the black 
...
to start. These amount of days is very short compared to the 30 days of the month 
(granularity used in the plots).
  ...
  plot_score_issue_deadline_date_month <- 
    df_score_issue_deadline_date_month %>% 
      ggplot(aes(x = issue_deadline_date, y = score)) +
        geom_col(alpha = .1, color = "grey50", width = 28) +
  ...
  plot_cum_score_issue_deadline_date_month <-
    df_cum_score_issue_deadline_date_month %>% 
      ggplot(aes(x = issue_deadline_date, y = cumscore)) +
        geom_area(alpha = .1, color = "grey50") +
        geom_smooth(method = "lm", size = .5, fill = "blue", alpha = .2) +
        scale_x_date(date_breaks = "3 month", date_labels = "%b %Y",
                     expand = c(0.02, 0)) +
        ...
        labs(title = "Cumulative Number of Issues Deadlines",
             subtitle = subtitle(nrow(df_issue_deadline_date_month)),
...
plot_issue_deadline_size <- function(df_issue_deadline_size) {
  summ_issue_deadline_size <- df_summary(df_issue_deadline_size, 
                                       from_start_to_deadline)
  
  plot_issue_deadline_size <-
    df_issue_deadline_size %>% 
      ggplot(aes(x = from_start_to_deadline)) +
        geom_histogram(binwidth = 1, color = "grey50", alpha = .1, 
                       boundary = summ_issue_deadline_size$min, 
                       closed = "left") +
        plot_x_summary(df_issue_deadline_size, from_start_to_deadline) +
        scale_x_continuous(breaks = seq(0, 45, 2)) +
        scale_y_continuous(limits = c(0, 400), breaks = seq(0, 400, 30)) +
        coord_cartesian(xlim = c(0, 45)) +
        labs(title = "Distribution of the Delay Days Between Start and Deadline Date",
...
df_issue_deadline_size <-
  issues %>% 
    ...
    mutate(from_start_to_deadline = difftime(issue_deadline_date,
                                             issue_start_date,
                                             units = "days")) %>% 
    filter(from_start_to_deadline >= 0)
```

```{r "Distribution of the Delay Days Between Start and Deadline Date"}
