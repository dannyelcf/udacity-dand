...
print(plot_number_logs_per_issue)
```
...
`issue_time_spent`. In an issue tracking system, timeline variables is the 
...
plot_cum_score_issue_created_by_month <-
  df_cum_score_issue_created_by_month %>% 
    ggplot(aes(x = issue_creation_date, y = cumscore)) +
      ...
      labs(title = "Cumulated Number of Issues Created per Month",
           subtitle = subtitle(nrow(df_issue_created_by_month)),
           x = "Issue creation date (per month)",
           y = "Number of issues") +
      theme(axis.text.x = element_text(angle = 40, hjust = 1)) 
...
```{r echo=FALSE, fig.retina=3, warning=FALSE, "Cumulated Number of Issues Created per Month"}
print(plot_cum_score_issue_created_by_month)
```
...
there was significant growth curve in demand (activities), approximately 25% of
growth over the previous 11 months. From June 2015 the demand growth seems to 
stabilize, sometimes taking small growth curves. In this period occurred 
...
```{r echo=FALSE, fig.retina=3, warning=FALSE, "Distribution of Number of Issues Created per Month"}
print(plot_score_issue_created_by_month)
```
...
print(rbind(y = summ_issue_created_by_wday))
print(plot_score_issue_created_by_wday)
```
...
```{r echo=FALSE, fig.retina=3, "Distribution of Number of Issues Started per Weekday"}
df_issue_start_wday <- 
  issues %>% 
    filter(!is.na(issue_start_date)) %>% 
    distinct(issue_id, 
...
```{r echo=FALSE, fig.retina=3, "Distribution of Number of Issues Deadline per Weekday"}
...
plot_issue_time_spent_min <- 
  df_issue_time_spent_min %>% 
    ggplot(aes(x = issue_time_spent)) +
      ...
      labs(title = "Distribution of Number of Time Spent in a Issue",
...
```{r echo=FALSE, fig.retina=3, "Distribution of Number of Time Spent in a Issue (Less Than 1 Hour and Non  0)"}
df_issue_time_spent_min <- 
  issues %>% 
    filter(!is.na(issue_time_spent)) %>%
    distinct(issue_id, issue_time_spent = (issue_time_spent / 60)) %>% 
...
```{r echo=FALSE, fig.retina=3, "Distribution of Number of Time Spent in a Issue (Non 0)"}
df_issue_time_spent_hour <- 
  issues %>% 
    filter(!is.na(issue_time_spent)) %>%
    distinct(issue_id, 
