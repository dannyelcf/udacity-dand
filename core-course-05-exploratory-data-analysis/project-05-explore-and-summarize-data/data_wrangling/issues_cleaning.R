...
issues <- 
  issues %>%
  mutate(issue_priority_scale = recode(issue_priority_scale,
                                       "SUSPENSA" = "SUSPENDED",
                                       "BAIXA" = "LOW", 
                                       "MEDIA" = "MEDIUM", 
                                       "ALTA" = "HIGH",
                                       "URGENTE" = "URGENT",
                                       "BLOQUEANTE" = "BLOCKING")) %>% 
  mutate(issue_priority_scale = factor(issue_priority_scale,
                                       levels = c("SUSPENDED",
                                                  "LOW", 
                                                  "MEDIUM", 
                                                  "HIGH",
...
issues <- 
  issues %>%
  mutate(issue_status = recode(issue_status,
                               "FINALIZADA" = "FINISHED",                            
                               "CANCELADA" = "CANCELED",                            
                               "RESOLVIDA" = "SOLVED",                             
...
# Cast issue_time_spent variable from character to Period
issues <- 
  issues %>%
  mutate(issue_time_spent = seconds_to_period(
    ifelse(str_detect(issue_time_spent, "h"),
           issue_time_spent %>% 
             str_replace(",", ".") %>% 
             str_replace("h", "") %>% 
             as.double() * 3600,
...
issues <- 
  issues %>%
  mutate(log_time_spent = hms(
    ifelse(str_detect(log_time_spent, "min"),
           log_time_spent %>% 
             paste("0s") %>%
             str_pad(8, pad ="0") %>% 
             str_pad(9, pad =" ") %>% 
...
issues <- 
  issues %>%
  mutate(issue_progress = issue_progress %>% 
           str_replace("%", "") %>% 
           as.integer(),
